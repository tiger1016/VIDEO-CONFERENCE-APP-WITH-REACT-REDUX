...
app.post('/', (req, res) => {
  if (roomOpen.length === 0) {
    roomOpen.push(req.body.room);
    res.status(200).json({ auth: 'presenter' });
  } else {
    if (roomOpen.indexOf(req.body.room) > -1) {
      res.status(200).json({ auth: 'listener' })
    }
  }
})
...
// app.get("/.well-known/pki-validation/E623A5C00A829DD4CDA5E739FB3BA512.txt", function (req, res) {
//   res.sendFile(path.join(public, ".well-known/pki-validation/E623A5C00A829DD4CDA5E739FB3BA512.txt"));
// });
...
// Listen for Heroku port, otherwise just use 80
